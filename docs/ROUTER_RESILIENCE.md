...
- 首个有意义 SSE 到达后，流式空闲 Hedge 按绝对阶段计时：6 分钟触发第一阶段、9 分钟触发第二阶段，最后再等待 3 分钟硬截止（实际阶段由 Runner 策略决定）。空事件、角色元数据和心跳不会刷新计时。
- 若下游停止读取但保持 TCP 连接，生成器可能卡在发送操作；独立的流式截止信号仍会关闭上游并发送终止 HTTP 504/SSE，避免 Trace 无限停留在 `running`。
