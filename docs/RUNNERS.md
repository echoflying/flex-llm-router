...
- **Runner** 以横向表格行展示，每个 Runner 独占一行；左侧为模型名、策略和地址信息，右侧为按顺序纵向排列的 Channel。模型名默认只读，悬停后可点“编辑”；Channel 可上移、下移或通过独立弹窗增加成员，新增项排在末尾，顺序就是调度/Hedge 的候选顺序。策略显示中文名称，`i` 图标查看说明，“更换”弹窗直接列出 Radio 选项切换既有策略；增加 Channel 弹窗按 Provider 分组并把 Provider 放在选项前面。页面可切换本机或局域网 Base URL，并复制 Base URL 与对外模型名。策略字段仍复用既有 Pool 策略。
